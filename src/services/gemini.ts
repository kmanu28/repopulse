export async function generateReadmeWithGemini(apiKey: string, context: any) {
  ...
  - Section 3: "Architecture Overview" with a clear, well-structured **Mermaid.js diagram** (\`\`\`mermaid) illustrating the system flow or component interactions. To prevent syntax errors, ALWAYS enclose node texts containing special characters or spaces in double quotes (e.g., \`A["User (Client)"] --> B("Server")\`). Include a brief text explanation below the diagram.
